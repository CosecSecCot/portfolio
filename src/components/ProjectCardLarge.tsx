...
export default function ProjectCardLarge({
  ...
  return (
    <div
      className={cn(
        "border border-foreground/20 flex max-sm:flex-col",
        className
      )}
    >
      ...
      <div className="flex-1 p-4 flex flex-col gap-[1em] justify-between">
        <div>
          ...
          <h2 className="text-xl lg:text-2xl">{project.title}</h2>
          <div className="mt-[0.5em] flex sm:flex-col max-md:gap-1 flex-wrap">
            {project.details.tools.map((tool, index) => (
              <p
                key={index}
                className="text-[0.875em] leading-tight text-neutral-400"
              >
                {tool}
